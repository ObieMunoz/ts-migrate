...
| [add-missing-imports](https://github.com/ObieMunoz/ts-migrate/blob/master/packages/ts-migrate-plugins/src/plugins/add-missing-imports.ts) | Import the names a file uses but never brings in, from the module TypeScript's own quick fix offers, instead of leaving each one to a suppression comment. A renamed file is full of them: a bundler injected the name as a global, a transform hoisted the `require` away, an earlier pass dropped the import. Every one is TS2304, and a suppressed name reads as `any` for the rest of the run, so the passes that infer from it lose the type too. The candidates are TypeScript's, asked for one diagnostic at a time, and the edits are its combined fix, which merges the names it takes from one module into a single statement and matches the quoting and specifier style the file already uses. A module and a barrel that re-exports it are one candidate, not two, since they reach the same export. Where the name really is exported by several modules the choice is a guess: `ambiguous: 'first'` (the default, and what "Add all missing imports" does in an editor) takes the one TypeScript ranks first, marks the import (see "Follow-up markers") with the modules it passed over, and reports it; `ambiguous: 'skip'` leaves the name for a person and lets ts-ignore suppress it. `moduleSpecifier` overrides how the specifier is written (`shortest`, `project-relative`, `relative`, `non-relative`). Only imports are ever added: a binding the combined fix produces that the candidate scan did not classify is taken back out before the file is written. |
...
| [react-class-state](https://github.com/ObieMunoz/ts-migrate/blob/master/packages/ts-migrate-plugins/src/plugins/react-class-state.ts) | Declare the state type of a React class component that does not have one. The members are the keys the class uses, gathered from `state` and `this.state` initializers, `setState` calls, and `this.state.key` writes in the same file; a member no initializer sets is optional, unless a statement of the constructor's own body writes it, which has run by the time the component exists. A member's type comes from the syntax of its value where that is enough (a literal, an array of them) and from the checker where it is not (a call, a shorthand naming a typed binding, a state object built by a function), with the names a resolved type spells imported. Where the checker resolves a type it cannot write back as text, an object shape or a function signature, the expression is named instead: `typeof defaultConfig` for an identifier, `ReturnType<typeof makeConfig>` for a call, and only for a binding declared at the top level of the same file. A type whose name the file has no way to reach, declared inside a function or private to a package, is refused rather than written as a name that resolves to nothing. Everything else takes `any` (`$TSFixMe`). |
...
| [update-import-paths](https://github.com/ObieMunoz/ts-migrate/blob/master/packages/ts-migrate-plugins/src/plugins/update-import-paths.ts) | Re-point imports that name a file under an extension the project does not ship it under: `./foo.js`/`./foo.jsx` after the file was renamed to `.ts`/`.tsx`, and `./foo.ts`/`./foo.tsx`, which is error TS5097 and gets stripped rather than suppressed. Drops the extension by default; keeps a `.js` extension in ESM packages (`"type": "module"`) or with `{ extension: 'js' }`. A `.cts`/`.cjs` file keeps the extensionless form even there: it emits `.cjs`, which is CommonJS whatever the package says. A `./foo.mts`/`./foo.cts` import becomes `./foo.mjs`/`./foo.cjs`, the extension it emits, either way. Imports whose target still exists on disk are left alone, as is a `.ts` extension under `allowImportingTsExtensions` or one whose base names a second file. Absolute imports the project maps through tsconfig `paths`, like `selectors/Address.js`, are re-pointed using the project's own module resolution; one the tsconfig cannot resolve is left alone. |
