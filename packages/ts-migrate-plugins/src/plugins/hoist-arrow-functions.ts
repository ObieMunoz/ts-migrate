...
import { hasModifier, hasOnlyModifier } from './utils/modifiers';
...
function getCandidate(
  ...
  if (!hasOnlyModifier(statement.modifiers, ts.SyntaxKind.ExportKeyword)) return undefined;
  ...
  if (!hasOnlyModifier(arrow.modifiers, ts.SyntaxKind.AsyncKeyword)) return undefined;
...
function toFunctionDeclarationText(
  ...
  const isExport = hasModifier(statement.modifiers, ts.SyntaxKind.ExportKeyword);
  const isAsync = hasModifier(arrow.modifiers, ts.SyntaxKind.AsyncKeyword);
