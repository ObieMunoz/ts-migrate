...
interface RunOptions {
  ...
  workingDir?: string;
  /**
   * Declaration files to check with the engine the pass resolved, the way a run
   * checks the ones its plugins generated. They are never written to the
   * fixture: ESLint resolves a config for a path, not for a file on disk.
   */
  generatedFiles?: { fileName: string; text: string }[];
}
...
interface FixtureRun {
  ...
  /** Set when the run passed generatedFiles; paths are relative to rootDir. */
  generatedFileParseErrors?: { fileName: string; message: string }[];
  ...
  try {
    ...
    const { status, signal, stdout, stderr } = spawnSync(
      ...
      [
        ...
        JSON.stringify({ files, rootDir, options: pluginOptions, generatedFiles }),
      ],
      {
...
describe('eslint-fix plugin', () => {
  ...
  it(
    'reports the generated declaration files the project config cannot parse',
    () => {
      // What the declare-globals plugin generates, whose `declare global` the
      // default parser stops at.
      const globals = [
        '// Generated by ts-migrate.',
        'export {};',
        '',
        'declare global {',
        '  interface Window {',
        '    captchaLoadCallback: any;',
        '  }',
        '}',
        '',
      ].join('\n');
      ...
      const { generatedFileParseErrors } = runInFixture(
        'eslint-flat-generated',
        [{ fileName: 'src/Foo.tsx', text: `const hello = 'world'` }],
        {
          generatedFiles: [
            { fileName: 'types/ts-migrate-globals.d.ts', text: globals },
            { fileName: 'types/ignored.d.ts', text: globals },
            { fileName: 'types/ts-migrate-modules.d.ts', text: "declare module 'a-package';\n" },
          ],
        },
      );

      // The ignored path is left out: a project that answered by ignoring the
      // generated declarations is not asked to answer again.
      expect(generatedFileParseErrors?.map(({ fileName }) => fileName)).toEqual([
        'types/ts-migrate-globals.d.ts',
        'types/ts-migrate-modules.d.ts',
      ]);
      expect(generatedFileParseErrors?.[0].message).toBe('Parsing error: Unexpected token global');
    },
    20000,
  );
  ...
  it(
    ...
    () => {
      const { generatedFileParseErrors, results } = runInFixture(
        ...
        [{ fileName: 'src/Foo.tsx', text: `const hello = 'world'` }],
        {
          // Nothing but an empty export: valid under either parser, so a
          // config that reads plain JavaScript is not a finding on its own.
          generatedFiles: [{ fileName: 'types/ts-migrate-globals.d.ts', text: 'export {};\n' }],
        },
      );
