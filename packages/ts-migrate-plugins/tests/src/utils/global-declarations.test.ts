...
`);
  ...
  it('skips element access, which names nothing to declare', () => {
    const evidence = collectText("window['byString'] = 1;\nwindow[key] = 2;");

    expect([...evidence.properties.keys()]).toEqual([]);
  });

  it('records the read a compound assignment and a nested write make', () => {
    const evidence = collectText(`
window.counter += 1;
...
`);
  ...
  it('collects a global nothing in the project assigns', () => {
    const rootDir = makeFixture();
    const evidence = collectText(`
window.vendorSdk.init();
if (window.featureFlags) {
  render(typeof window.featureFlags);
}
const stamp = globalThis.buildStamp;
`);

    expect(declarationsIn(evidence, rootDir)).toEqual([
      { name: 'buildStamp', target: 'globalThis', type: 'any' },
      { name: 'featureFlags', target: 'window', type: 'any' },
      { name: 'vendorSdk', target: 'window', type: 'any' },
    ]);
    expect(evidence.properties.get('featureFlags')!.reads).toBe(2);
  });

  it('does not let a read widen the type the assignments derived', () => {
    const rootDir = makeFixture();
    const evidence = collectText(`
window.enabled = true;
if (window.enabled) {
  report(window.enabled);
}
`);

    expect(declarationsIn(evidence, rootDir)).toEqual([
      { name: 'enabled', target: 'window', type: 'boolean' },
    ]);
    const property = evidence.properties.get('enabled')!;
    expect(property.assignments).toBe(1);
    expect(property.reads).toBe(2);
    expect(property.unknownType).toBe(false);
  });

  it('collects the read on the right of an assignment, including the polyfill idiom', () => {
    const evidence = collectText('window.copy = window.source;\nwindow.queue = window.queue || [];');

    expect([...evidence.properties.keys()].sort()).toEqual(['copy', 'queue', 'source']);
    expect(evidence.properties.get('source')!.assignments).toBe(0);
    const queue = evidence.properties.get('queue')!;
    expect(queue.assignments).toBe(1);
    expect(queue.reads).toBe(1);
  });

  it('skips a read whose root an enclosing scope binds', () => {
    const evidence = collectText(`
(function (window) {
  send(window.fromIife);
})(fakeWindow);
function withLocal() {
  const global = {};
  return global.fromLocal;
}
`);
  ...
  it('does not treat an `in` check as a read, since it is not an error', () => {
    const evidence = collectText("if ('maybeThere' in window) { use(); }");

    expect([...evidence.properties.keys()]).toEqual([]);
  });

  it('leaves the properties the environment already declares alone when they are read', () => {
    const evidence = collectFrom({
      '/a.ts': `
const href = window.location.href;
const title = window.name;
const ok = globalThis.isNaN(1);
window.vendorSdk.init();
`,
    });

    expect([...evidence.properties.keys()]).toEqual(['vendorSdk']);
  });

  it('declares a property read through globalThis as a var', () => {
    const evidence = collectText("window.shared = 'a';\nconst copy = globalThis.shared;");

    expect(evidence.properties.get('shared')!.target).toBe('globalThis');
  });

  it('leaves a read of a name no var can have alone', () => {
    const evidence = collectText('use(globalThis.import);\nuse(globalThis.registry);');
...
`,
  ...
  it('type-check with no error at the sites that use them', () => {
...
describe('createGlobalDeclarations', () => {
  ...
  it('accumulates across files and writes the file once', async () => {
    ...
    expect(collector.summarize()).toEqual({
      ...
      properties: [
        { name: 'appName', assignments: 2, reads: 0, fileCount: 2 },
        { name: 'buildId', assignments: 1, reads: 0, fileCount: 1 },
      ],
...
describe('formatGlobalDeclarationsReport', () => {
  it('names each declaration and where it is used', () => {
    const report = formatGlobalDeclarationsReport({
      ...
      declarations: [
        ...
        { name: 'grecaptcha', target: 'window', type: 'any' },
      ],
      properties: [
        { name: 'appConfig', assignments: 4, reads: 7, fileCount: 2 },
        { name: 'buildId', assignments: 1, reads: 0, fileCount: 1 },
        { name: 'grecaptcha', assignments: 0, reads: 3, fileCount: 2 },
      ],
    });

    expect(report).toContain('3 globals declared in types/ts-migrate-globals.d.ts');
    expect(report).toContain('appConfig: any (4 assignments, 7 reads in 2 files)');
    expect(report).toContain('buildId: string (1 assignment in 1 file)');
    expect(report).toContain('grecaptcha: any (3 reads in 2 files, never assigned)');
